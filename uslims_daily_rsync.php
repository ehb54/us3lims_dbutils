...
if ( isset( $rsync_add ) ) {
    ...
    backup_rsync_run_cmd( $cmd, false );
    $cmd = "sudo rsync -azL --no-specials --no-devices -e 'ssh -i /home/usadmin/.ssh/id_rsa -l usadmin' --rsync-path='sudo rsync' $rsync_add_details $rsync_user@$rsync_host:$rsync_add_path/$use_backup_host 2>&1 >> $logf";
    backup_rsync_run_cmd( $cmd );
}
